...
MultibodyPlant<T>::MultibodyPlant(double time_step)
  ...
  // By default, the model config is empty and therefore the solver determines
  // the contact model.
  DRAKE_DEMAND(contact_model_enum_ == DiscreteContactModel::kTamsi);
  DRAKE_DEMAND(MultibodyPlantConfig{}.discrete_contact_model == "");
}
...
template <typename T>
void MultibodyPlant<T>::set_discrete_contact_model(
    DiscreteContactModel contact_model) {
  DRAKE_MBP_THROW_IF_FINALIZED();
  contact_model_enum_ = contact_model;
  if (contact_model == DiscreteContactModel::kTamsi) {
    set_discrete_contact_solver(DiscreteContactSolver::kTamsi);
  } else {
    set_discrete_contact_solver(DiscreteContactSolver::kSap);
  }
}

template <typename T>
DiscreteContactModel MultibodyPlant<T>::get_discrete_contact_model() const {
  return contact_model_enum_;
}
...
template <typename T>
void MultibodyPlant<T>::set_sap_sigma(double sigma) {
  DRAKE_MBP_THROW_IF_FINALIZED();
  sap_sigma_ = sigma;
}

template <typename T>
double MultibodyPlant<T>::get_sap_sigma() const {
  return sap_sigma_;
}

template <typename T>
double MultibodyPlant<T>::get_margin() const {
  return margin_;
}

template <typename T>
void MultibodyPlant<T>::set_margin(double margin) {
  DRAKE_MBP_THROW_IF_FINALIZED();
  margin_ = margin;
}
