...
void DynamicBvh::ResetMovingQueryCached() {
  std::deque<DynamicBvNode*> q;
  q.push_back(&*root_node_);
  while (!q.empty()) {
    DynamicBvNode* node = q.front();
    node->reset_moving_query_cached();
    if (!node->is_leaf()) {
...
void DynamicBvh::SetMovingFrames(const RigidTransformd& X_WA0,
                                 const RigidTransformd& X_WA1) {
  this->ResetMovingQueryCached();
  this->moving_frames_set_ = true;
  this->X_WA0_ = &X_WA0;
  this->X_WA1_ = &X_WA1;
  this->R0_abs_ = X_WA0.rotation().matrix().cwiseAbs();
  this->R1_abs_ = X_WA1.rotation().matrix().cwiseAbs();
  this->CacheMovingQueryAabb(mutable_root_node());
}

void DynamicBvh::CacheMovingQueryAabb(DynamicBvNode& node) {
  DRAKE_DEMAND(this->moving_frames_set_);
  const Aabb bv0((*X_WA0_) * (node.bv().center()),
                 R0_abs_ * (node.bv().half_width()));
  const Aabb bv1((*X_WA1_) * (node.bv().center()),
                 R1_abs_ * (node.bv().half_width()));
  node.setMovingQueryBV(Aabb(bv0, bv1));
}

void DynamicBvh::CollideMoving(DynamicBvh& bvh_B,
                               DynamicBvttCallback callback) {
  // Check for the case of self collision.
  if (&bvh_B == this) {
    throw std::logic_error("DynamicBvh::CollideMoving() not supported for self collision.");
  }

  DRAKE_DEMAND(this->moving_frames_set_);
  DRAKE_DEMAND(bvh_B.moving_frames_set_);

  using NodePair = std::pair<DynamicBvNode&, DynamicBvNode&>;
  std::stack<NodePair> node_pairs;
  node_pairs.emplace(mutable_root_node(), bvh_B.mutable_root_node());

  while (!node_pairs.empty()) {
    const auto& [node_a, node_b] = node_pairs.top();
    node_pairs.pop();

    if (!node_a.moving_query_cached()) {
      this->CacheMovingQueryAabb(node_a);
    }
    if (!node_b.moving_query_cached()) {
      bvh_B.CacheMovingQueryAabb(node_b);
    }

    // Check if the bounding volumes overlap.
    if (!Aabb::HasOverlap(node_a.moving_query_bv(), node_b.moving_query_bv())) {
      continue;
    }
...
std::vector<std::pair<int, int>> DynamicBvh::GetMovingCollisionCandidates(
    DynamicBvh& bvh_B) {
  DRAKE_DEMAND(this->moving_frames_set_);
  DRAKE_DEMAND(bvh_B.moving_frames_set_);
  std::vector<std::pair<int, int>> result;
