#include "drake/multibody/contact_solvers/sap/sap_contact_surface_constraint.h"
...
template <typename T>
SapContactSurfaceConstraint<T>::SapContactSurfaceConstraint(
...
template <typename T>
SapContactSurfaceConstraintData<T>::SapContactSurfaceConstraintData(
...
template <typename T>
std::unique_ptr<AbstractValue> SapContactSurfaceConstraint<T>::DoMakeData(
    const T& time_step,
  ...
  SapContactSurfaceConstraintData<T> data(parameters_.mu, Rt, Rn, vn_hat);
  return SapConstraint<T>::MoveAndMakeAbstractValue(std::move(data));
}

template <typename T>
ContactMode SapContactSurfaceConstraint<T>::CalcContactMode(const T& mu,
                                                          const T& mu_hat,
                                                          const T& yr,
                                                          const T& yn) {
...
template <typename T>
void SapContactSurfaceConstraint<T>::DoCalcData(
    const Eigen::Ref<const VectorX<T>>& vc,
    AbstractValue* abstract_data) const {
  auto& data =
      abstract_data->get_mutable_value<SapContactSurfaceConstraintData<T>>();
...
template <typename T>
void SapContactSurfaceConstraint<T>::ProjectImpulse(
    const SapContactSurfaceConstraintData<T>& data, Vector3<T>* gamma) const {
...
template <typename T>
T SapContactSurfaceConstraint<T>::DoCalcCost(
    const AbstractValue& abstract_data) const {
  const auto& data =
      abstract_data.get_value<SapContactSurfaceConstraintData<T>>();
...
template <typename T>
void SapContactSurfaceConstraint<T>::DoCalcImpulse(
...
template <typename T>
void SapContactSurfaceConstraint<T>::DoCalcCostHessian(
...
template <typename T>
void SapContactSurfaceConstraint<T>::DoAccumulateSpatialImpulses(
