...
class SpatialForce : public SpatialVector<SpatialForce, T> {
 ...
 public:
  ...
  // We define the product of a rigid transform on a spatial force as a shift
  // operation followed by a re-expression as:
  //  F_BPo_P = X_PM * F_BMo_M 
  //          = F_BMo_P.Shift(p_PM) = (R_PM * F_BMo_M).Shift(p_PM)
  friend SpatialForce<T> operator*(const math::RigidTransform<T>& X_PM,
                                   const SpatialForce<T>& F_BMo_M) {
    const Vector3<T> p_MP_P = -X_PM.translation();
    const math::RotationMatrix<T>& R_PM = X_PM.rotation();
    return (R_PM * F_BMo_M).Shift(p_MP_P);
  }
