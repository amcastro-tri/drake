...
#define PRINT_VEC(a) std::cout << #a": " << a.transpose() << std::endl;
...
ContactSolverStatus MpConvexSolver<double>::SolveWithGuess(
  ...
  const VectorX<double> y = -(Rinv.asDiagonal() * (vc - vc_stab));
...
typename MpConvexSolver<T>::PreProcessedData MpConvexSolver<T>::PreProcessData(
  ...
  if (nc != 0) {
    ...
    for (int ic = 0, ic3 = 0; ic < nc; ic++, ic3 += 3) {
      ...
      Vector3<T> b(0.0, 0.0, phi0(ic) / time_step);
      vc_stab.template segment<3>(ic3) = -b;
    }
    data.r = data.vc_star - vc_stab;
    PRINT_VEC(data.vc_star);
    PRINT_VEC(vc_stab);
    PRINT_VEC(data.r);
...
void MpConvexSolver<T>::SetUpSolver(
  ...
  for (int ic = 0, ic3 = 0; ic < nc; ic++, ic3 += 3) {
    ...
    // constraints. Therefore we use Lorent constraint below.
    //auto cone_constraint = std::make_shared<FrictionConeConstraint>(mu(ic));
...
#if 0
template <typename T>
void MpConvexSolver<T>::CalcInitialCondition(const VectorX<T>& vc,
                                                  VectorX<T>* gamma) const {
  using std::abs;
  using std::max;
  using std::sqrt;
