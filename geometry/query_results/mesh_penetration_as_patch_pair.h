...
/** @name     Triangle mesh penetration reported as set of point-pairs
 ...
 following quantities:
   ...
   - The barycentric coordinates for a with respect to triangle Tᵃ: αᵃᵢ, βᵃ,
     γᴬ. If a is a node of mesh A, then one of αᵃ, βᵃ, γᵃ will be one
     and the other values will be zero. In any case, they should sum to one.
   - A triangle on which point bᵢ lies, called Tᵇ.
   ...
   - The outward-pointing unit normal n̂ᵃ of mesh A at point a. Generally,
     `(b - a) · nᵃ ≠ φ`.
   - The outward-pointing unit normal n̂ᵇᵢ of mesh B at point bᵢ.
 ...
 <!--
    General assumptions:
      ...
      2. The mesh provides per-node normals. These normals are interpolated
...
template <typename T>
struct PenetrationSamplePointPair {
  DRAKE_DEFAULT_COPY_AND_MOVE_AND_ASSIGN(PenetrationSamplePointPair)
...
/** The characterization of penetration between two *meshes* as a set of sampled
 pair points spanning the overlapping volume. Provides the identifiers for the
 two penetrating mesh geometries and the point pairs values.

 @tparam T The underlying scalar type. Must be a valid Eigen scalar.  */
template <typename T>
struct MeshPenetrationAsSamplePointPairs {
  DRAKE_DEFAULT_COPY_AND_MOVE_AND_ASSIGN(MeshPenetrationAsSamplePointPairs)
  ...
  /** The collection of sampled point pairss on the overlapping volume of meshes
   A and B.  */
  const std::vector<PenetrationSamplePointPair> point_pairs;
};
