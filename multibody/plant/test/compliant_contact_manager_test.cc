 ...
 public:
  ...
  void SetUp() override {
    ...
    ConstraintIndex constraint_index = plant_.AddCouplerConstraint(
        left_finger_slider, right_finger_slider, gear_ratio, offset);
...
TEST_F(KukaIiwaArmTests, CouplerConstraints) {
  ...
  const Vector1d g0_expected(q_left - gear_ratio * q_right - offset);
  ...
  const MatrixXd J_expected =
      ...
      gear_ratio * VectorXd::Unit(kNumJoints, right_index)).transpose();
