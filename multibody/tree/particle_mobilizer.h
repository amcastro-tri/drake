...
template <typename T>
class ParticleMobilizer final : public MobilizerImpl<T, 3, 3> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(ParticleMobilizer)

  /* Constructor for a %ParticleMobilizer between an inboard frame F
  ...
  ParticleMobilizer(const Frame<T>& inboard_frame_F,
  ...
  const ParticleMobilizer<T>& set_position(
  ...
  /* Sets in `context` the rate of change, in meters per second, of `this`
   ...
   @returns A constant reference to `this` mobilizer. */
  const ParticleMobilizer<T>& set_translational_velocity(
      systems::Context<T>* context,
