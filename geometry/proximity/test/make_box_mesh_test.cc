...
// MakeRefinedBoxVolumeMeshWithMa() satisfies all these properties:
//
...
GTEST_TEST(MakeRefinedBoxVolumeMeshWithMa, ParameterValueCoverage) {
  // A box with a point MA (same three dimensions).
  {
    const Box box(2., 2., 2.);
    const VolumeMesh<double> mesh = MakeRefinedBoxVolumeMeshWithMa<double>(box);
    EXPECT_EQ(mesh.num_elements(), 24);
  ...
  // A box with a line MA (two dimensions are the same).
  {
    ...
    for (const Box& box : {Box{L2, L1, L1}, Box{L1, L2, L1}, Box{L1, L1, L2}}) {
      const VolumeMesh<double> mesh =
          MakeRefinedBoxVolumeMeshWithMa<double>(box);
