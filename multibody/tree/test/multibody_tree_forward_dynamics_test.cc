...
class KukaIiwaModelForwardDynamicsTests : public ::testing::Test {
 public:
  ...
  void CalcForwardDynamicsViaArticulatedBodyAlgorithm(
    ...
    const auto& pc = tree().EvalPositionKinematics(*context_);
    const auto& vc = tree().EvalVelocityKinematics(*context_);
    ...
    tree().CalcForwardDynamics(*context_, forces, &ac);
    *vdot = ac.get_vdot();
  }
  ...
  void CalcForwardDynamicsViaMassMatrixSolve(
    ...
    tree().CalcInverseDynamics(
        *context_, VectorX<double>::Zero(nv), Fapplied_Bo_W_array,
