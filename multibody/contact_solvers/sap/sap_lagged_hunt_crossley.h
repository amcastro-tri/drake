...
template <typename T>
class SapLaggedHuntCrossley final : public SapConstraint<T> {
 public:
  DRAKE_DEFAULT_COPY_AND_MOVE_AND_ASSIGN(SapLaggedHuntCrossley);
  ...
  struct Parameters {
    ...
    T hunt_crossley_dissipation{0.0};
  ...
  SapLaggedHuntCrossley(int clique, MatrixX<T> J, const T& x0, const T& xdot0,
  ...
  /* Constructs a contact constraint for the case in which two cliques
   ...
   @param[in] phi0 The value of the signed distance at the previous time step.
   @param[in] parameters Constraint parameters. See Parameters for details. */
  SapLaggedHuntCrossley(int clique0, int clique1, MatrixX<T> J0, MatrixX<T> J1,
                        const T& x0, const T& xdot0, const Parameters& p);
  ...
  std::unique_ptr<SapConstraint<T>> Clone() const final {
    return std::make_unique<SapLaggedHuntCrossley<T>>(*this);
  }
 ...
 private:
  ...
  void DoInitializeParameters(
      const T& time_step,
      const Eigen::Ref<const VectorX<T>>& delassus_estimation) final;
  T DoCalcCost(const Eigen::Ref<const VectorX<T>>& vc) const final;
  void DoCalcImpulse(const Eigen::Ref<const VectorX<T>>& vc,
                     EigenPtr<VectorX<T>> gamma) const final;
  void DoCalcCostHessian(const Eigen::Ref<const VectorX<T>>& vc,
                         MatrixX<T>* G) const final;  

  // Compute gamma(x, xdot) for the Hunt & Crossley model with stiff core.
  T CalcHuntCrossleyImpulse(const T& x, const T& xdot) const;

  // Computes discrete cost ell(v) = -N(vn; x0).
  T CalcDiscreteHuntCrossleyCost(const T& vn) const;

  // Computes discrete impulse function n(vn; x0)
  T CalcDiscreteHuntCrossleyImpulse(const T& vn) const;

  // Computes gradient of the discrete impulse function, n'(vn; x0)
  // This returns Gn = -dn/dvn > 0.
  // N.B. dn/dvn = -dn/dxdot
  T CalcDiscreteHuntCrossleyImpulseGradient(const T& vn) const;

  Parameters parameters_;
  T x0_;
  T xdot0_;

  // Barrier exponential parameter. Zero for quadratic barrier.
  T lambda_{1.0};

  // Parameters only valid after InitializeParameters().
  T kMax_;       // Maximum stiffness computed from Delassus approximation. 
  T kEff_;       // Effective stiffness used in the model, kEff = min(kMax, k).
  T epsilon_soft_;  // Softnening parameters.
  T time_step_;
  T kappa_;
  T x_min_;
};
