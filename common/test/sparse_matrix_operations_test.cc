...
GTEST_TEST(EigenSparseVector, AddVectorsWithSamePattern) {
  SparseVectord x(6);
  x.coeffRef(0) = 0.5;
  x.coeffRef(2) = 1.5;
  PRINT_DENSE_VEC(x);
...
GTEST_TEST(EigenSparseVector, AddVectorsWithDifferentPattern) {
  SparseVectord x(6);
  x.coeffRef(0) = 0.5;
  x.coeffRef(2) = 1.5;
  PRINT_DENSE_VEC(x);
...
GTEST_TEST(EigenSparseVector, TimesScalar) {
  SparseVectord x(6);
  x.coeffRef(0) = 0.5;
  x.coeffRef(2) = 1.5;
  PRINT_DENSE_VEC(x);
  ...
  const SparseVectord s_times_x = s * x;
  PRINT_DENSE_VEC(s_times_x);

  const SparseVectord x_times_s = x * s;
  PRINT_DENSE_VEC(x_times_s);
}
