 ...
 * @return true if the two matrices are equal based on the specified tolerance.
 */
template <class DerivsVector>
bool CompareMatrices(const Eigen::Ref<const MatrixX<Eigen::AutoDiffScalar<DerivsVector>>>& m1,
                     const Eigen::Ref<const MatrixX<Eigen::AutoDiffScalar<DerivsVector>>>& m2,
                     double tolerance,
                     MatrixCompareType compare_type,
                     std::string* explanation = nullptr) {
  bool result = true;
