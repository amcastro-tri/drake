...
class DynamicBvNode {
 public:
  ...
  DynamicBvNode(Aabb bv, LeafData data)
      : bv_(std::move(bv)),
        moving_query_bv_(Vector3d::Zero(), Vector3d::Zero()),
  ...
  bool moving_query_cached() const { return moving_query_cached_; }
  void reset_moving_query_cached() { moving_query_cached_ = false; }

  const Aabb& moving_query_bv() const {
    DRAKE_DEMAND(moving_query_cached_);
    return moving_query_bv_;
  }

  void setBV(Aabb bv) { bv_ = std::move(bv); }
  void setMovingQueryBV(Aabb bv) {
    moving_query_bv_ = std::move(bv);
    moving_query_cached_ = true;
  }
 ...
 private:
  ...
  Aabb bv_;
  Aabb moving_query_bv_;
  bool moving_query_cached_{false};
...
class DynamicBvh {
 public:
  ...
  // Moving query functions.
  bool moving_frames_set() const { return moving_frames_set_; }

  void SetMovingFrames(const RigidTransformd& X_WA0,
                       const RigidTransformd& X_WA1);

  void CollideMoving(DynamicBvh& bvh_B, DynamicBvttCallback callback);
  ...
  std::vector<std::pair<int, int>> GetMovingCollisionCandidates(
      DynamicBvh& bvh_B);
 ...
 private:
  ...
  void CacheMovingQueryAabb(DynamicBvNode& node);
  void ResetMovingQueryCached();

  DynamicBvNode& mutable_root_node() { return *root_node_; }
  copyable_unique_ptr<DynamicBvNode> root_node_;
  int num_leaves_{};
  bool moving_frames_set_{false};
  const RigidTransformd* X_WA0_{nullptr};
  const RigidTransformd* X_WA1_{nullptr};
  Matrix3d R0_abs_;
  Matrix3d R1_abs_;
};
