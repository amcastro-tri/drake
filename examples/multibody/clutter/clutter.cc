...
DEFINE_double(stiff_core_depth, 10.0, "Stiff core depth [m].");
...
DEFINE_string(discrete_contact_model, "sap",
              "Discrete contact solver. Options are: 'tamsi', 'sap', 'lagged', "
              "'convex'.");
DEFINE_double(near_rigid_threshold, 1.0, "SAP near rigid threshold.");
DEFINE_double(sdf_max_distance, 0.05,
              "Sdf max distance. Zero for penetration only queries.");
DEFINE_double(sap_sigma, 0.0,
              "SAP regularization of friction in the H&C models.");

DEFINE_bool(
    estimate_cond_number, false,
    "Whether to estimate SAP's condition number, at the last iteration");
...
int do_main() {
  ...
  plant_config.discrete_contact_model = FLAGS_discrete_contact_model;
  plant_config.sdf_max_distance = FLAGS_sdf_max_distance;
  plant_config.sap_sigma = FLAGS_sap_sigma;
  ...
  sap_params.max_iterations = 500;
  sap_params.estimate_cond_number = FLAGS_estimate_cond_number;
  sap_params.cost_rel_tolerance = 0.0;
  sap_params.cost_abs_tolerance = 0.0;
