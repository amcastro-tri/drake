...
std::unique_ptr<Joint<ToScalar>> ParticleJoint<T>::TemplatedDoCloneToScalar(
  ...
  auto joint_clone = std::make_unique<ParticleJoint<ToScalar>>(
...
template <typename T>
std::unique_ptr<Joint<double>> ParticleJoint<T>::DoCloneToScalar(
    const internal::MultibodyTree<double>& tree_clone) const {
  return TemplatedDoCloneToScalar(tree_clone);
}

template <typename T>
std::unique_ptr<Joint<AutoDiffXd>> ParticleJoint<T>::DoCloneToScalar(
    const internal::MultibodyTree<AutoDiffXd>& tree_clone) const {
  return TemplatedDoCloneToScalar(tree_clone);
}

template <typename T>
std::unique_ptr<Joint<symbolic::Expression>> ParticleJoint<T>::DoCloneToScalar(
...
ParticleJoint<T>::MakeImplementationBlueprint() const {
  auto blue_print = std::make_unique<typename Joint<T>::BluePrint>();
  auto mobilizer = std::make_unique<internal::ParticleMobilizer<T>>(
      this->frame_on_parent(), this->frame_on_child());
  mobilizer->set_default_position(this->default_positions());
  blue_print->mobilizers_.push_back(std::move(mobilizer));
  return blue_print;
}
