...
bool RigidBodyTree::transformCollisionFrame(
  ...
  return collision_model_->transformCollisionFrame(eid, transform_body_to_joint);
}
  ...
  for (int i = 0; i < closest_points.size(); ++i) {
    ...
    const RigidBody::CollisionElement* elementB =
        dynamic_cast<const RigidBody::CollisionElement*>(
            collision_model_->readElement(closest_points[i].getIdB()));
...
bool RigidBodyTree::collisionDetect(
  ...
  // collision_model_->closestPointsAllToAll" << endl;
  // END_DEBUG
  bool points_found =
      collision_model_->closestPointsAllToAll(ids_to_check, use_margins, points);
  // DEBUG
  ...
  for (int i = 0; i < points.size(); ++i) {
    ...
    const RigidBody::CollisionElement* elementA =
        dynamic_cast<const RigidBody::CollisionElement*>(
            collision_model_->readElement(points[i].getIdA()));
    // DEBUG
    // cout << "RigidBodyTree::collisionDetect: points[i].getIdA() = " <<
    // points[i].getIdA() << endl;
    // cout << "RigidBodyTree::collisionDetect:
    // collision_model_->readElement(points[i].getIdA()) = " <<
    // collision_model_->readElement(points[i].getIdA()) << endl;
    // cout << "RigidBodyTree::collisionDetect:
    // collision_model_->readElement(points[i].getIdA())->getId() = " <<
    // collision_model_->readElement(points[i].getIdA())->getId() << endl;
    ...
    const RigidBody::CollisionElement* elementB =
        dynamic_cast<const RigidBody::CollisionElement*>(
            collision_model_->readElement(points[i].getIdB()));
...
void RigidBodyTree::potentialCollisions(const KinematicsCache<double>& cache,
  ...
  potential_collisions = collision_model_->potentialCollisionPoints(use_margins);
  ...
  for (size_t i = 0; i < num_potential_collisions; i++) {
    const RigidBody::CollisionElement* elementA =
        dynamic_cast<const RigidBody::CollisionElement*>(
            collision_model_->readElement(potential_collisions[i].getIdA()));
    const RigidBody::CollisionElement* elementB =
        dynamic_cast<const RigidBody::CollisionElement*>(
            collision_model_->readElement(potential_collisions[i].getIdB()));
...
bool RigidBodyTree::collidingPointsCheckOnly(
  ...
  updateDynamicCollisionElements(cache);
  return collision_model_->collidingPointsCheckOnly(points, collision_threshold);
}
...
bool RigidBodyTree::allCollisions(const KinematicsCache<double>& cache,
  ...
  vector<DrakeCollision::PointPair> points;
  bool points_found =
      collision_model_->collisionPointsAllToAll(use_margins, points);
