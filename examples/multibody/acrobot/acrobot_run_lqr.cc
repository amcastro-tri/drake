...
class AcrobotWithNoOutputs : public systems::Diagram<T> {
 public:
  AcrobotWithNoOutputs()
      : Diagram<T>(systems::SystemTypeTag<drake::examples::multibody::acrobot::AcrobotWithNoOutputs>{}) { // Should I namespace it?????
    ...
    builder.ExportInput(acrobot_->get_actuation_input_port());
    builder.BuildInto(this);
  }

  // Scalar-converting copy constructor.
  template <typename U>
  explicit AcrobotWithNoOutputs(const AcrobotWithNoOutputs<U>& other)
      : Diagram<T>(other) {}
...
int main(int argc, char* argv[]) {
  ...
  return drake::examples::multibody::acrobot::do_main(argc, argv);
}
