...
class SpatialVelocity : public SpatialVector<SpatialVelocity, T> {
 ...
 public:
  ...
  /// We define the application of a rigid transform transpose to a spatial
  /// velocity as the combination of a shift operation followed by a re-express
  /// operation, i.e.:
  ///   V_WPb_B = X_PBᵀ * V_WP_P = R_PBᵀ * V_WP_P.Shift(p_PB_B).
  friend SpatialVelocity<T> operator*(
      const math::RigidTransformTranspose<T>& X_PB_T,
      const SpatialVelocity<T>& V_WP_P) {
    const math::RigidTransform<T>& X_PB = X_PB_T.pose();
    const Vector3<T>& p_PB = X_PB.translation();
    const math::RotationMatrix<T>& R_PB = X_PB.rotation();
    return R_PB.transpose() * V_WP_P.Shift(p_PB);
  }

  // For completeness, we also implement the version of this operator when the
  // X_BP is given in terms of an explicitly constructed transform (instead of
  // its transpose). Though the operation is the same as above, the
  // implementation changes to make use of the available data:
  //   V_WPb_B = X_BP * V_WP_P = V_WP_B.Shift(-p_BP_B)
  //           = (R_BP * V_WP_P).Shift(-p_BP_B).
  friend SpatialVelocity<T> operator*(const math::RigidTransform<T>& X_BP,
                                      const SpatialVelocity<T>& V_WP_P) {
    const Vector3<T>& p_BP = X_BP.translation();
    const math::RotationMatrix<T>& R_BP = X_BP.rotation();
    return (R_BP * V_WP_P).Shift(-p_BP);
  }
